...
import { createUserWithEmailAndPassword } from "firebase/auth";
  ...
  const Index = () => {
    ...
    const changeImage = async () => {
      const desertRef = ref(storage, `physio/${formData.images.name}`);
      try {
        const res = await deleteObject(desertRef);
        console.log(res, "res");
        handleUploadImage();
    ...
    const addData = async (imageUrl, id) => {
      ...
      try {
        ...
        setBtnAdd("primary");
        MySwal.fire({
          title: <p>Physio added SuccessFully</p>,
          didOpen: () => {
    ...
    const updateData = async (docs) => {
      ...
      try {
        ...
        await updateDoc(doc(db, "physio", formData.id), { ...formData });
        if (inUpdate) {
          await updateDoc(doc(db, "physio", formData.id), {
    ...
    const deleteData = async (e) => {
      ...
      try {
        ...
        MySwal.fire({
          title: <p>Physio Deleted SuccessFully</p>,
          didOpen: () => {
            // `MySwal` is a subclass of `Swal` with all the same instance & static methods
            MySwal.showLoading()
          },
    ...
    const handleUploadImage = async (id) => {
      setLoading(true);
      const imgRef = ref(storage, `physio/${imgUrl ? imgUrl.name : ""}`);
    ...
    const fetchData = async () => {
      let arr = [];
      const dbRef = collection(db, "physio");
      try {
    ...
    const { isLoading, data, error } = useQuery(["Physio"], fetchData, {
      staleTime: 60000,
    });
    ...
    return (
      <div className="flex flex-col bg-white space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="font-semibold font-barlow text-2xl ml-6">Physio</h1>
          <button
