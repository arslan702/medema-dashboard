...
const Index = () => {
  ...
  const delteImage = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "doctors", formData.id), { images: {} });
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const addData = async (imageUrl) => {
    // setBtnAdd("disable");
    const url = imageUrl;
    const newCityRef = collection(db, "doctors");
    ...
    try {
      ...
      await updateDoc(doc(db, "doctors", res.id), {
        id: res.id,
      ...
      setBtnAdd("primary");
      MySwal.fire({
        title: <p>Doctor added SuccessFully</p>,
        didOpen: () => {
  ...
  const updateData = async (docs) => {
    ...
    try {
      if (inUpdate) {
        const desertRef = ref(storage, `doctors/${formData.images.name}`);
        const deleteImg = await deleteObject(desertRef);
        console.log(deleteImg);
      }
      await updateDoc(doc(db, "doctors", formData.id), { ...formData });
      if (inUpdate) {
  ...
  const deleteData = async (e) => {
    console.log(e);
    try {
      const desertRef = ref(storage, `doctors/${e.images.name}`);
      const deleteImg = await deleteObject(desertRef);
      const res = await deleteDoc(doc(db, "doctors", e.id));
      console.log(res);
      console.log(deleteImg);
      setAlertType("error");
  ...
  const handleUploadImage = async () => {
    setLoading(true);
    const imgRef = ref(storage, `doctors/${imgUrl ? imgUrl.name : ""}`);
  ...
  const fetchData = async () => {
    let arr = [];
    const dbRef = collection(db, "doctors");
    try {
  ...
  const { isLoading, data, error } = useQuery(["doctors"], fetchData, {
    staleTime: 60000,
  });
  ...
  return (
    <div className="flex flex-col bg-white space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="font-semibold font-barlow text-2xl ml-6">Doctors</h1>
        <button
