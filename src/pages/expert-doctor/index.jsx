...
useEffect(()=>{
 if(formData.hospital!==""){
  let newCat = hospitalName?.data?.find((doc)=>{
  ...
  setSpecial( newCat?newCat.category:[])
    ...
    const handleUploadImage = async (id) => {
      setLoading(true);
      const imgRef = ref(storage, `expert-doctor/${imgUrl ? imgUrl.name : ""}`);
    ...
    //     const dbRef = collection(db, "expert");
    //     try {
    //       const res = await query(dbRef,where('name',"==",formData.hospital));
    //       res.docs.map((doc) => {
    //         arr.push(doc.data());
    //       });
    //       return arr;
    ...
    //     }
    //   };
    //   const categoryName = useQuery(["expertCategory"], fetchCategory, {
    //     staleTime: 60000,
    //   });
    ...
    return (
      <div className="flex flex-col bg-white space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="font-semibold font-barlow text-2xl ml-6">Expert Doctors</h1>
          <button
          ...
          <form onSubmit={handleFormSubmit}>
            <div className="grid grid-cols-12 gap-x-4 gap-y-3">
              ...
              <div className="col-span-6 flex space-x-2">
                ...
                <div>
                <Select
                  className="w-[145px] px-2 ml-6 rounded-md text-black"
                  name="gender"
                  placeholder="Gender"
              ...
              <div className="col-span-6 flex justify-between  ">
                ...
                >
                  ...
                  {
                    hospitalName?.data?.map((doc,index)=>{
                        return(
                            index==0?
              ...
              <div className="col-span-6 flex space-x-2">
                ...
                <Select
                  className="w-[150px] px-2 rounded-md text-black"
                  name="speciality"
