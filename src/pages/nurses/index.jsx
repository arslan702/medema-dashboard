...
const Index = () => {
  ...
  const addData = async (imageUrl, uid) => {
    ...
    try {
      ...
      setBtnAdd("primary");
      MySwal.fire({
        title: <p>Nurse added SuccessFully</p>,
        didOpen: () => {
  ...
  const deleteData = async (e) => {
    ...
    try {
      ...
      MySwal.fire({
        title: <p>Nurse Deleted SuccessFully</p>,
        didOpen: () => {
          // `MySwal` is a subclass of `Swal` with all the same instance & static methods
          MySwal.showLoading()
        },
  ...
  const handleUploadImage = async (uid) => {
    setLoading(true);
    const imgRef = ref(storage, `nurses/${imgUrl ? imgUrl.name : ""}`);
  ...
  return (
    <div className="flex flex-col bg-white space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="font-semibold font-barlow text-2xl ml-6">Nurse</h1>
        <button
